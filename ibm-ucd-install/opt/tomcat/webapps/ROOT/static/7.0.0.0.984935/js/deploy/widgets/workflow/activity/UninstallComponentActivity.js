...
function(
    ...
    return declare('deploy.widgets.workflow.activity.UninstallComponentActivity',  [BaseActivity], {
        postCreate: function() {
        ...
        getLabel: function() {
            ...
            if (!this.initialized) {
                result += i18n("Uninstall Component");
            }
            else {
                ...
                if (childChildData.component && childChildData.component.deleted) {
                    ...
                    result += this.getDisplayName(childChildData.name, childChildData.componentName,
                            i18n("Uninstall"), i18n("Uninstall %s", childChildData.componentName));
        ...
        editProperties: function(callback) {
            ...
            propertyForm.addField({
                ...
                description: i18n("The maximum number of concurrent processes to run, per component. " +
                        "This setting limits the number of processes that run simultaneously to " +
                        "uninstall each component. For example, if you set the maximum number of " +
                        "concurrent processes to 2, only two instances of a component are uninstalled " +
                        "at a time, even if many instances of the component are currently installed. " +
                        "Use -1 for unlimited.")
            });
        ...
        fdwShowFieldsForComponent: function(propertyForm, component) {
            ...
            if (component) {
                ...
                var componentProcessSelect = new WebextSelect({
                    ...
                    selectOnClick: true,
                    allowNone: false,
                    noDataMessage: i18n("No component processes found."),
                    value: initialComponentProcess
                });
                propertyForm.addField({
                    name: "componentProcessName",
        ...
        getDefaultName: function(componentName) {
            return i18n("Uninstall: \"%s\"", componentName);
        }
    });
});
