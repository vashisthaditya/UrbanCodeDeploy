...
function(
    ...
    return declare('deploy.widgets.deploymentPreview.ResourceChanges',  [_Widget, _TemplatedMixin], {
        templateString: 
            '<div class="resourceChanges">' + 
        ...
        postCreate: function() {
            ...
            var gridLayout = [{
                ...
                formatter: function(item, result, cellDom) {
                    var name;
                    if (item.component) {
                        name = formatters.componentLinkFormatter(item.component);
                    }
                    else {
                        name = util.getOnClickLink("resource/"+item.id, item.path);
                    }
                    return name;
                },
            ...
            if (self.applicationProcessRequest) {
                tableUrl = bootstrap.restUrl+"deploy/applicationProcessRequest/"+self.applicationProcessRequest.id+"/changesByResource";
            }
            else {
                tableUrl = bootstrap.restUrl+"deploy/snapshot/"+self.snapshot.id+"/changesByResource"+
