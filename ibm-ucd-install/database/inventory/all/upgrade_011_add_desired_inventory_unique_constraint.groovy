...
def selectAllTablesZOS = '''
    select * from SYSIBM.SYSTABLES where dbname = ?
'''

def selectAllIndexesForTable = '''
    select * from SYSIBM.SYSINDEXES where tbname = ?
'''
...
def dbType = properties["database.type"];
def dbName = properties["database.databasename"];
...
// db2zos is a special case since DatabaseMetadata.getTables and getIndexInfo
// return empty ResultSets
if (dbType.equals("db2zos")) {
    def tablePrepStatement = sql.createPreparedQueryCommand(selectAllTablesZOS, [dbName])
    def zosTableRS = tablePrepStatement.execute();
    while (zosTableRS.next()) {
        def zosTableName = zosTableRS.getString('NAME')
        if (zosTableName.equalsIgnoreCase('inv_desired_inventory')) {
            inventoryTableName = zosTableName
            break;
        }
    }
    tablePrepStatement.closeResources(zosTableRS)

    def indexPrepStatement = sql.createPreparedQueryCommand(selectAllIndexesForTable, [inventoryTableName])
    def zosIndexRS = indexPrepStatement.execute();
    while (zosIndexRS.next()) {
        def constraintName = zosIndexRS.getString('NAME')
        println "Index found: ${constraintName}"
...
while (inventoryTableName == null && tableRS.next()) {
...
while (!constraintFound && indexRS.next()) {
