...
boolean propertyShouldBeSecure(String propertyName) {
    return SystemConfiguration.getSecurePropertyNames().contains(propertyName);
}
...
final String getSystemConfigurationProperties = '''
SELECT *
FROM ps_prop_value
WHERE prop_sheet_id = '00000000-0000-0000-0000-000000000001'
'''
...
sql.eachRow(getSystemConfigurationProperties) { row ->
    String propName = row['name'];
    String propValue = (String)row['value'];
    if (propertyShouldBeSecure(propName) && !CryptStringUtil.isEncrypted(propValue)) {
        String encryptedValue = CryptStringUtil.encrypt(propValue);
        sql.executeUpdate(updateWithEncryptedValue, [encryptedValue, row['id']]);
    }
}

// remove outdated, unused WinRS properties
sql.executeUpdate('''
DELETE
FROM ps_prop_value
WHERE prop_sheet_id = '00000000-0000-0000-0000-000000000001'
AND name = 'server.winrs.pass'
OR name = 'server.winrs.user'
''');
