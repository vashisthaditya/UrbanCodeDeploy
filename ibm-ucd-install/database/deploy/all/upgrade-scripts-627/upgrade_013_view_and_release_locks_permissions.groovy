...
def createViewLocksPermissionSQL = '''
...
def createReleaseLocksPermissionSQL = '''
    insert into sec_action(id, version, name, description, enabled, cascading, sec_resource_type_id, category)
...
sql.execute(createViewLocksPermissionSQL)
sql.execute(createReleaseLocksPermissionSQL)

List<String> roles = getRoleIds()

for (String role : roles) {
    sql.execute(addActionSQL, UUID.randomUUID().toString(), role, '200000000000000000000000001b000b', null)
    sql.execute(addActionSQL, UUID.randomUUID().toString(), role, '200000000000000000000000001b000c', null)
}