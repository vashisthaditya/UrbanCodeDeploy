...
'''select * from (
    select id, persistent_data, row_number() over() as r
    from vc_persistent_record ) as tableName
where r > ? and r <= ?'''

String selectSecurePropDef =
'''select * from (
    select id,default_value,long_default_value,property_type, row_number() over() as r 
    from ps_prop_def ) as tableName
where property_type='SECURE' and r > ? and r <= ?'''

String selectSecureUsers =
'''select * from (
    select id,password, row_number() over() as r
    from sec_user ) as tableName
where r > ? and r <= ?'''

String selectAgentData =
'''select * from (
    select agent_data, row_number() over() as r 
    from ds_agent_data ) as tableName
where r > ? and r <= ?'''

String selectResource =
'''select * from (
    select id,impersonation_password, row_number() over() as r
    from ds_resource ) as tableName
where r > ? and r <= ?'''

String selectAgents =
'''select * from (
    select id, impersonation_password, row_number() over() as r
    from ds_agent ) as tableName
where r > ? and r <= ?'''
